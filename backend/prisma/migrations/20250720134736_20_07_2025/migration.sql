...
CREATE TABLE `Message` (
    ...
    `expediteurId` INTEGER UNSIGNED NOT NULL,
    `destinataireId` INTEGER UNSIGNED NOT NULL,
...
CREATE TABLE `HistoriquePrestataire` (
    ...
    UNIQUE INDEX `HistoriquePrestataire_clientId_prestataireId_key`(`clientId`, `prestataireId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `Message` ADD CONSTRAINT `Message_rendezVousId_fkey` FOREIGN KEY (`rendezVousId`) REFERENCES `RendezVous`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Notification` ADD CONSTRAINT `Notification_rendezVousId_fkey` FOREIGN KEY (`rendezVousId`) REFERENCES `RendezVous`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
